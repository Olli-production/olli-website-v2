...
Operi all'interno di un'architettura a 3 livelli che separa le responsabilit per massimizzare l'affidabilit. Gli LLM sono probabilistici, mentre la maggior parte della logica di business  deterministica e richiede coerenza. Questo sistema risolve il problema.
...
- Leggi le direttive, chiama gli strumenti di esecuzione nell'ordine giusto, gestisci gli errori, chiedi chiarimenti, aggiorna le direttive con ci che impari
...
**Perch funziona:** se fai tutto tu stesso, gli errori si sommano. 90% di accuratezza per step = 59% di successo su 5 step. La soluzione  spingere la complessit in codice deterministico. Cos tu ti concentri solo sul decision-making.
...
- Aggiorna la direttiva con ci che hai imparato (limiti API, timing, casi limite)
...
Gli errori sono opportunit di apprendimento. Quando qualcosa si rompe:
1. Correggilo
2. Aggiorna il tool
3. Testa il tool, assicurati che funzioni
4. Aggiorna la direttiva per includere il nuovo flusso
5. Il sistema ora  pi forte
...
- **Deliverable**: Google Sheets, Google Slides o altri output cloud-based a cui l'utente pu accedere
...
**Principio chiave:** I file locali sono solo per l'elaborazione. I deliverable vivono nei servizi cloud (Google Sheets, Slides, ecc.) dove l'utente pu accedervi. Tutto in `.tmp/` pu essere cancellato e rigenerato.
